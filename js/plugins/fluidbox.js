	...
	"use strict";
		...
		$.extend(Plugin.prototype, {
			...
			open: function() {
                ...
                $fb			= $(this.element),
                $fbThumb	= $fb.find('img').first(),
                $fbLrgLink  = $fbThumb.data('fullsize'),
                $fbGhost	= $fb.find('.fluidbox__ghost'),
                $fbWrap		= $fb.find('.fluidbox__wrap');

                var bgc = getAverageRGB($fb.find('img')[0]);
                
				var fullsize    = $fb.attr('href');
                // var fullsize    = $fbThumb.data('fullsize') ? $fbThumb.data('fullsize') : $fb.attr('href');
				...
				var $fbOverlay = $('<div />', {
					...
					css: {
						zIndex: -1,
                        backgroundColor: 'rgb(' + bgc.r + ',' + bgc.g + ',' + bgc.b + ')'
					}
				});
			...
			compute: function() {
				var fb			= this,
                    ...
                    // .on('click', function() {
                    //     window.open($fbLrgLink);
                    // });
                    // $('#custom-trigger-1-social')
                    // .find('span.small')
                    // .on('click', function() {
                    //     window.open($fbSmlLink);
                    // });
